...
func TestParseWKBPoint(t *testing.T) {
    // WKB for POINT(1.0 2.0)
    data := []byte{
    ...
    x, y, err := ParseWKBPoint(data)
    require.NoError(t, err)
    require.Equal(t, 1.0, x)
    require.Equal(t, 2.0, y)
}

func TestParseWKBPolygon(t *testing.T) {
    data := []byte{
        0x01,                   // Byte order (little-endian)
        0x03, 0x00, 0x00, 0x00, // Type (POLYGON)
        0x01, 0x00, 0x00, 0x00, // Num rings (1)
        0x04, 0x00, 0x00, 0x00, // Num points (4)
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // 0,0
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, 0x3F, // 1,0
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, 0x3F, // 1,1
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // 0,0
    }
    rings, err := ParseWKBPolygon(data)
    require.NoError(t, err)
    expected := [][][2]float64{
        {{0, 0}, {1, 0}, {1, 1}, {0, 0}},
    }
    require.Equal(t, expected, rings)
}